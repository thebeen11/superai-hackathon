  ...
  return (
    <div
      style={{
        ...
        fontWeight: 700,
        fontSize: size * (glyphLen > 1 ? 0.34 : 0.44),
        color: txt,
      ...
      <div style={{ textAlign: "right" }}>
        <div
          ...
          style={{
            fontSize: 13,
            fontWeight: 600,
            color: a.queue ? "var(--t-hi)" : "var(--t-lo)",
          }}
        >
          {a.queue}
        </div>
        <div className="label-xs" style={{ fontSize: 8.5 }}>
          queue
        </div>
      </div>
    </button>
            ...
            <div
              style={{
                ...
                background:
                  p.state === "pending"
                    ? "transparent"
                    : `color-mix(in oklch, ${ac.c} 15%, transparent)`,
            ...
            <div style={{ flex: 1 }}>
              <div
                style={{
                  fontSize: 12.5,
                  fontWeight: 500,
                  color: p.state === "pending" ? "var(--t-lo)" : "var(--t-hi)",
                }}
              >
                {p.stage}
              </div>
...
export function AgentDrawer({
  ...
  // Newest-first, matching the Command Center Activity Log card.
  const ordered = [...liveEntries].reverse();
  return (
    <div
      onClick={onClose}
      style={{
    ...
    >
      <div
        ...
        style={{
          ...
          height: "100%",
          background: "var(--bg-1)",
          borderLeft: "1px solid var(--stroke-hi)",
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div
              className="mono"
              style={{ fontSize: 11.5, color: "var(--t-mid)" }}
            >
              {agent.role}
            </div>
          </div>
        ...
        >
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 16 }}>
            ...
            <div style={{ textAlign: "right" }}>
              <div
                className="mono"
                style={{ fontSize: 15, fontWeight: 600, color: ac.c }}
              >
                {agent.throughput}
              </div>
              <div className="label-xs" style={{ fontSize: 8 }}>
        ...
        <Section label={hasLive ? "Activity log · live" : "Activity log"}>
          {hasLive ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {liveEntries.map((e, i) => {
                const etone = ACCENTS[STATE_TONE[e.status]]
                  ? STATE_TONE[e.status]
                  : null;
                const color =
                ...
                return (
                  ...
                  >
                    ...
                    {etone && (
                      <span style={{ marginLeft: "auto", paddingTop: 4 }}>
                        <Dot tone={etone} pulse={false} />
                      </span>
                    )}
                  </div>
                );
      ...
      <div style={{ position: "relative" }}>
        ...
        <span
          style={{
            ...
            height: 9,
            borderRadius: 99,
            background: ACCENTS[tone] ? ACCENTS[tone].c : tone,
            border: "2px solid var(--bg-1)",
            animation: status !== "idle" ? "pulse-dot 1.6s infinite" : "none",
          }}
        />
      </div>
      <span
        className="mono"
        style={{ fontSize: 9, color: "var(--t-lo)", whiteSpace: "nowrap" }}
      >
        {a.label}
      </span>
    </button>
  ...
  return (
    <div style={{ position: "relative", paddingRight: 30 }}>
      ...
      {tiers.map((t, i) => {
        ...
        const liveMembers = t.squad
          .map((a) => liveStatus?.[a.id])
          .filter(Boolean) as LiveAgentStatus[];
        const activeLive = liveMembers.find((l) => l.status !== "idle");
        const tStatus = activeLive
          ? activeLive.status
          : liveMembers.length
            ? "idle"
            : t.status;
        const tStatusText = activeLive
          ? activeLive.statusText
          : liveMembers.length
            ? "done"
            : t.statusText;
            ...
            >
              <div
                style={{
                  ...
                  display: "grid",
                  placeItems: "center",
                  background: `color-mix(in oklch, ${ac.c} 14%, transparent)`,
                  border: `1px solid color-mix(in oklch, ${ac.c} 45%, transparent)`,
                }}
              >
                <span
                  className="mono"
                  style={{
                    fontSize: 11,
                    fontWeight: 700,
                    color: t.accent === "chair" ? "var(--t-hi)" : ac.c,
                  }}
                >
                  T{t.n}
                </span>
