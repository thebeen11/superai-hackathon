...
export function CouncilCard({
  ...
  return (
    <Card
      title="Agent Council"
      sub={`${tiers.length} tiers · ${agentCount} agents`}
      className="span8"
      action={
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <Dot tone={discovering ? "orange" : "blue"} pulse={discovering} />
          <span
            className="mono"
            style={{ fontSize: 11, color: "var(--t-mid)" }}
          >
            {discovering ? "running" : "DAG active"}
          </span>
        </div>
      }
    >
      <TierFlow
        ...
        onOpenAgent={(a: Agent) => onOpenAgent(a.id)}
      />
    </Card>
  );
}
