...
export function SourcesPage() {
  ...
  const pageItems = sources.slice(safePage * PAGE_SIZE, safePage * PAGE_SIZE + PAGE_SIZE);
      ...
      <div className="grid12">
        <Card title="Data Sources" sub={`${sources.filter((s) => s.live).length} live`} className="span6"
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {pageItems.map((s, i) => (
              <div key={s.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < pageItems.length - 1 ? "1px solid var(--stroke)" : "none" }}>
